...
Dialog_InsertFrames::Dialog_InsertFrames(Gtk::Window& parent,Canvas::Handle canvas)
    ...
    canvas_(canvas)
{
    ...
    apply_button->signal_clicked().connect(sigc::mem_fun(*this, &Dialog_InsertFrames::on_apply_pressed));

    Gtk::Button *cancel_button(manage(new Gtk::Button(_("_Close"), true)));
    cancel_button->show();
    add_action_widget(*cancel_button,0);
    cancel_button->signal_clicked().connect(sigc::mem_fun(*this, &Dialog_InsertFrames::hide));

    Gtk::Button *ok_button(manage(new Gtk::Button(_("_OK"), true)));
    ok_button->show();
    add_action_widget(*ok_button,2);
    ok_button->signal_clicked().connect(sigc::mem_fun(*this, &Dialog_InsertFrames::on_ok_pressed));

    show_all_children();
}

void Dialog_InsertFrames::set_canvas(synfig::Canvas::Handle x)
{
    canvas_ = x;
...
void Dialog_InsertFrames::set_time(synfig::Time time)
{
    time_widget_->set_value(time);
}

Time Dialog_InsertFrames::get_time() const
{
    return time_widget_->get_value();
}

Time Dialog_InsertFrames::get_duration() const
{
    return duration_widget_->get_value();
}

void Dialog_InsertFrames::on_ok_pressed()
{
    hide();
    signal_edited_();
}

void Dialog_InsertFrames::on_apply_pressed()
{
    signal_edited_();
}
